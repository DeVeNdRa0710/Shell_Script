...
host="localhost"
port=5000
slack_webhook_url="https://hooks.slack.com/services/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"
...
send_slack_alert(){
	...
	if [ $? -eq 0 ]; then
		echo "$timestamp: Slack Alert sent successfully" >> "$log_file"
	else
		echo "$timestamp: Failed to send Slack alert" >> "$log_file"
	fi
}
